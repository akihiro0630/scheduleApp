    ...
    private void prepData(HttpServletRequest request){
        ...
        for (int num = 1; num <= maxday; num++) {
            if(num < startday || num > lastday + startday - 1){
            	...
            	//押下した日付を送る
            	String ymd = Integer.valueOf(year).toString() + String.format("%02d",month) +  String.format("%02d",date);
                sb.append("<td><form class =\"sample\" action=\"/schedule/ScheduleAccsess\" method=\"post\">" + "<input type=\"hidden\" name=\"ymd\" value=" +"\""+ ymd + "\">" + "<input type=\"submit\" value = \""+ date +"\">"+ "</form></td>");
                date++;
            }
            if(num % 7 == 0){
                sb.append("</tr>");
